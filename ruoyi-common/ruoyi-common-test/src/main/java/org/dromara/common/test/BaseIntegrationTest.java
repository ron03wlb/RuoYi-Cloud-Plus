 ...
 * <h3>重要说明：</h3>
 * <p>
 * 本类使用 <b>手动容器启动</b> 方式（方案 A）以解决 Dubbo + Testcontainers 时序冲突问题。
 * 容器在静态初始化块中启动，确保在 Spring Boot 的 BeanFactoryPostProcessor 阶段之前容器已经运行。
 * 这样 Dubbo 的 DubboContextPostProcessor 扫描环境属性时，容器端口已经可用。
 * </p>
 *
 * @author Lion Li
 * @since 2025-11-09
...
public abstract class BaseIntegrationTest {
     ...
     * 动态配置数据源和 Redis 连接属性
     * <p>
     * 容器已在静态初始化块中启动，此方法将容器配置注入到 Spring Boot 属性中。
     * 由于容器已经运行，{@code getMappedPort()} 等方法可以安全调用。
     *
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("redisson.single-server-config.address", () ->
            "redis://" + REDIS_CONTAINER.getHost() + ":" +
                REDIS_CONTAINER.getMappedPort(TestContainersConfig.Redis.getPort()));

        // 配置 MinIO
        registry.add("oss.endpoint", () ->
            "http://" + MINIO_CONTAINER.getHost() + ":" +
                MINIO_CONTAINER.getMappedPort(TestContainersConfig.MinIO.getApiPort()));
