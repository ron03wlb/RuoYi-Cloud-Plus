...
public class TestContainersConfig {
    ...
    public static class MinIO {
        ...
        @SuppressWarnings("resource")
        public static GenericContainer<?> createContainer() {
            GenericContainer<?> minio = new GenericContainer<>(DockerImageName.parse(MINIO_IMAGE))
                .withExposedPorts(MINIO_API_PORT, MINIO_CONSOLE_PORT)
                .withEnv("MINIO_ROOT_USER", ACCESS_KEY)
                .withEnv("MINIO_ROOT_PASSWORD", SECRET_KEY)
                .withCommand("server", "/data", "--console-address", ":" + MINIO_CONSOLE_PORT)
                .withReuse(true)
                .withLogConsumer(new Slf4jLogConsumer(log));
    ...
    public static class RocketMQ {
        ...
        public static GenericContainer<?> createNameServerContainer() {
            GenericContainer<?> nameServer = new GenericContainer<>(DockerImageName.parse(ROCKETMQ_IMAGE))
     ...
     * Nacos 容器配置（可选）
     * <p>
     * 注意：Nacos 容器启动较慢，建议使用 Mock 配置代替
     */
    public static class Nacos {
        ...
        @SuppressWarnings("resource")
        public static GenericContainer<?> createContainer() {
            GenericContainer<?> nacos = new GenericContainer<>(DockerImageName.parse(NACOS_IMAGE))
