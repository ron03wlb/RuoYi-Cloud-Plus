...
class IntegrationTestFrameworkValidationTest extends BaseIntegrationTest {
    ...
    void shouldCreateTempTable() {
      ...
      String name =
          jdbcTemplate.queryForObject(
              "SELECT name FROM test_framework_validation WHERE id = 1", String.class);
  ...
  class SqlScriptExecutorTests {
    ...
    void shouldTruncateTable() {
      ...
      JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
      Integer count =
          jdbcTemplate.queryForObject("SELECT COUNT(*) FROM test_truncate", Integer.class);
    ...
    void shouldCheckTableExists() {
      ...
      // Arrange
      SqlScriptExecutor.executeSql(dataSource, "CREATE TABLE IF NOT EXISTS test_exists (id INT)");
