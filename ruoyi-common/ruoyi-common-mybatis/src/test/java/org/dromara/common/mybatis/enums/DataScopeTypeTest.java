...
class DataScopeTypeTest {
  ...
  class EnumConstantsTests {
    ...
    void shouldHaveDeptAndChildConstant() {
      ...
      assertThat(DataScopeType.DEPT_AND_CHILD.getSqlTemplate()).contains("#{@sdss.getDeptAndChild");
  ...
  class SqlTemplateValidationTests {
    ...
    void customShouldUseServiceCall() {
      assertThat(DataScopeType.CUSTOM.getSqlTemplate()).contains("@sdss").contains("getRoleCustom");
    }
    ...
    void deptAndChildOrSelfShouldContainOrOperator() {
      assertThat(DataScopeType.DEPT_AND_CHILD_OR_SELF.getSqlTemplate()).containsIgnoringCase("OR");
    }
  ...
  class CodeValidationTests {
    ...
    void allConstantsShouldHaveUniqueCode() {
      assertThat(DataScopeType.values()).extracting(DataScopeType::getCode).doesNotHaveDuplicates();
    }
    ...
    void allCodesShouldBeNumericStrings() {
      for (DataScopeType type : DataScopeType.values()) {
        assertThat(type.getCode()).as("DataScopeType %s 的 code 应该是数字", type).matches("\\d+");
      }
    }
    ...
    void allCodesShouldNotBeNullOrEmpty() {
      for (DataScopeType type : DataScopeType.values()) {
        assertThat(type.getCode()).as("DataScopeType %s 的 code 不应该为 null 或空", type).isNotEmpty();
      }
    }
  }
