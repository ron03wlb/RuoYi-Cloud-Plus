...
public class DataPermissionPointcut extends StaticMethodMatcherPointcut {
  ...
  public boolean matches(Method method, Class<?> targetClass) {
    ...
    if (Proxy.isProxyClass(targetClassRef)) {
      // 数据权限注解不对继承生效，但由于 SpringIOC 容器拿到的实际上是 MyBatis 代理过后的 Mapper，而
      // targetClass.isAnnotationPresent 实际匹配的是 Proxy 类的注解，不会查找代理类。
      // 所以这里不能用 targetClass.isAnnotationPresent，只能用 AnnotatedElementUtils.hasAnnotation 或
      // targetClass.getInterfaces()[0].isAnnotationPresent 去做匹配，以检查被代理的 MapperClass 是否具有注解
