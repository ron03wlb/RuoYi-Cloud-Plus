...
class ServiceExceptionTest {
  ...
  class MessageFormattingTests {
    ...
    void shouldHandleLongMessageFormatting() {
      ...
      // Act
      ServiceException exception =
          new ServiceException(template, "张三", "2024-10-30 10:00:00", "删除", "用户数据", "权限不足");
  ...
  class RealWorldScenarioTests {
    ...
    void shouldHandlePermissionDenied() {
      ...
      assertThat(exception.getDetailMessage()).isEqualTo("用户 admin 尝试访问 /system/user/delete 接口");
    }
    ...
    void shouldHandleValidationError() {
      // Act
      ServiceException exception =
          new ServiceException("数据验证失败：字段{}的值{}不符合规则{}", "email", "invalid-email", "必须是有效的邮箱地址");

      // Assert
      assertThat(exception.getMessage()).isEqualTo("数据验证失败：字段email的值invalid-email不符合规则必须是有效的邮箱地址");
    }
