...
class SqlUtilTest extends BaseUnitTest {
  ...
  class FilterKeywordTest {
    ...
    void shouldPassForNormalData() {
      ...
      assertThatCode(() -> SqlUtil.filterKeyword("normal-data_123")).doesNotThrowAnyException();
    }

    @ParameterizedTest
    @ValueSource(strings = {"select * from users", "SELECT * FROM users", "SeLeCt * FrOm users"})
    ...
    @ParameterizedTest
    @ValueSource(
        strings = {"mid (database(), 1, 1)", "MID (database(), 1, 1)", "select mid from table"})
    ...
    @ParameterizedTest
    @ValueSource(strings = {"truncate table users", "TRUNCATE TABLE users", "TrUnCaTe TaBlE users"})
