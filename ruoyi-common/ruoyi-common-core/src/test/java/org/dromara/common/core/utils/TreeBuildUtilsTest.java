 ...
 * TreeBuildUtils 测试类.
 *
 * <p>测试树形结构构建工具类的各种功能，包括： - 基本树构建 - 多根节点树构建 - 叶子节点提取 - 边界条件处理
 *
 * @author Test Team
 */
class TreeBuildUtilsTest extends BaseUnitTest {
  ...
  void shouldBuildSingleNodeTree() {
    List<TreeNode> nodes = Arrays.asList(new TreeNode(1L, 0L, "根节点"));
  ...
  void shouldBuildMultipleRootTrees() {
    ...
    // 验证树的结构
    Tree<Long> tree1 = result.stream().filter(t -> t.getId().equals(1L)).findFirst().orElse(null);
    assertThat((Object) tree1).isNotNull();
    assertThat(tree1.getName()).isEqualTo("树1根节点");
    assertThat(tree1.getChildren()).hasSize(2);

    Tree<Long> tree2 = result.stream().filter(t -> t.getId().equals(4L)).findFirst().orElse(null);
  ...
  void shouldReturnOnlyLeafNodesFromTwoLevelTree() {
    ...
    assertThat(leafNodes).extracting(Tree::getId).containsExactlyInAnyOrder(2L, 3L, 4L);
  }
  ...
  void shouldReturnOnlyLeafNodesFromMultiLevelTree() {
    ...
    assertThat(leafNodes).hasSize(3);
    assertThat(leafNodes).extracting(Tree::getId).containsExactlyInAnyOrder(4L, 5L, 7L);
  }
  ...
  void shouldExtractLeafNodesFromMultipleRootTrees() {
    ...
    assertThat(leafNodes).hasSize(3);
    assertThat(leafNodes).extracting(Tree::getId).containsExactlyInAnyOrder(2L, 3L, 6L);
  }
