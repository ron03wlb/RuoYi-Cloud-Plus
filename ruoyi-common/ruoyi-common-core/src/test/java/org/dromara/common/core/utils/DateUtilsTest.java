...
class DateUtilsTest extends BaseUnitTest {
  ...
  void shouldGetTimeDifferenceWithOnlyHours() {
    ...
    assertThat(diff).isEqualTo("5小时").doesNotContain("天").doesNotContain("分钟").doesNotContain("秒");
  }
  ...
  void shouldHandleNullDateInFormatMethods() {
    ...
    assertThatThrownBy(() -> DateUtils.formatDate(null)).isInstanceOf(NullPointerException.class);
