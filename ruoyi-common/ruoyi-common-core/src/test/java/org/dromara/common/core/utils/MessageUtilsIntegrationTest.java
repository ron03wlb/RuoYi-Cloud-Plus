...
class MessageUtilsIntegrationTest extends BaseIntegrationTest {
  ...
  class RealScenarioTest {
    ...
    void shouldHandleUserLoginErrorScenario() {
      ...
      // Act & Assert
      assertThat(MessageUtils.message("user.password.not.match")).isEqualTo("用户名或密码错误");

      assertThat(MessageUtils.message("user.jcaptcha.error")).isEqualTo("验证码错误");

      assertThat(MessageUtils.message("user.jcaptcha.expire")).isEqualTo("验证码已失效");
    }
    ...
    void shouldHandlePasswordRetryLimitScenario() {
      ...
      // Act & Assert - 第1次错误
      assertThat(MessageUtils.message("user.password.retry.limit.count", 1)).isEqualTo("密码输入错误1次");

      // 第3次错误
      assertThat(MessageUtils.message("user.password.retry.limit.count", 3)).isEqualTo("密码输入错误3次");
  ...
  class BoundaryTest {
    ...
    void shouldHandleSpecialCharacterArguments() {
      ...
      // Act
      String message =
          MessageUtils.message("user.password.retry.limit.count", "<script>alert('xss')</script>");
