...
class SseExceptionTest {
  ...
  class EdgeCaseTests {
    ...
    void shouldHandleUnicodeCharacters() {
      ...
      // Assert
      assertThat(exception.getMessage()).contains("😀").contains("©").contains("®").contains("™");
    }
  ...
  class RealWorldSseScenarioTests {
    ...
    void shouldHandleMessagePushFailure() {
      // Act
      SseException exception = new SseException().setMessage("消息推送失败").setDetailMessage("网络I/O异常");
    ...
    void shouldHandleConnectionLimitExceeded() {
      // Act
      SseException exception =
          new SseException("SSE连接数已达上限", 1004).setDetailMessage("当前连接数: 1000, 最大连接数: 1000");
      ...
      assertThatThrownBy(
              () -> {
                try {
                  ...
                  throw new SseException("SSE层异常", 500).setDetailMessage("原因: " + e.getMessage());
                }
              })
          .isInstanceOf(SseException.class)
          .hasMessage("SSE层异常")
