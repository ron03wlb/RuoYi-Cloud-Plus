 ...
 * AddressUtils 测试类
 * <p>
 * 注意：由于 RegionUtils 在静态初始化时会加载 ip2region.xdb 文件，
 * 且该文件在测试环境中不存在，因此本测试只覆盖不依赖 RegionUtils 的逻辑。
 * 涉及 RegionUtils 的测试需要在集成测试中完成。
 *
 * @author Test Team
 */
...
class AddressUtilsTest {
    ...
    class GetRealAddressByIPBoundaryTest {
        ...
        void shouldHandleEmptyStringInput() {
            assertThat(AddressUtils.getRealAddressByIP(""))
