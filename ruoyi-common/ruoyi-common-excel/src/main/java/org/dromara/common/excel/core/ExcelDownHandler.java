...
public class ExcelDownHandler implements SheetWriteHandler {
    ...
    public void afterSheetCreate(
        ...
        for (Map.Entry<Integer, FieldWrapper> entry : fieldCache.getSortedFieldMap().entrySet()) {
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                List<Object> values =
                        EnumUtil.getFieldValues(format.enumClass(), format.textField());
                ...
                everyOptions -> {
                    ...
                    if (!everyOptions.getNextOptions().isEmpty()) {
                        ...
                        dropDownWithSheet(
                                ...
                                workbook,
                                sheet,
                                everyOptions.getIndex(),
                                everyOptions.getOptions());
                    } else {
                        // 否则使用默认形式
                        dropDownWithSimple(
                                helper, sheet, everyOptions.getIndex(), everyOptions.getOptions());
                    }
                });
    }
        ...
        for (int columIndex = 0; columIndex < columnNames.size(); columIndex++) {
            ...
            String sonFunction =
                    String.format(
                            ...
                            Math.max(
                                            Optional.ofNullable(
                                                            secoundOptionsMap.get(
                                                                    thisFirstOptionsValue))
    ...
    private void dropDownWithSheet(
        ...
        Sheet simpleDataSheet =
                Optional.ofNullable(
                                workbook.getSheet(
                                        WorkbookUtil.createSafeSheetName(tmpOptionsSheetName)))
                        .orElseGet(
                                () ->
        ...
        // 设置数据校验为序列模式，引用的是名称管理器中的别名
        this.markOptionsToSheet(
                helper, sheet, celIndex, helper.createFormulaListConstraint(nameName));
