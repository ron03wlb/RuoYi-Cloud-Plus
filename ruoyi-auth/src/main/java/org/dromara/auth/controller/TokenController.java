...
/**
 * Token controller for handling authentication, login, logout, and registration operations.
 *
 * @author Lion Li
 */
@Slf4j
...
public class TokenController {
  ...
  public R<LoginVo> login(@RequestBody String body) {
    ...
    log.debug(
        "Login request received: clientId={}, grantType={}, tenantId={}",
        clientId,
        grantType,
        tenantId);
    ...
    if (ObjectUtil.isNull(clientVo) || !StringUtils.contains(clientVo.getGrantType(), grantType)) {
      log.warn(
          "Invalid client authentication: clientId={}, grantType={},"
              + " reason=client_not_found_or_grant_type_mismatch",
          clientId,
          grantType);
      return R.fail(MessageUtils.message("auth.grant.type.error"));
    } else if (!SystemConstants.NORMAL.equals(clientVo.getStatus())) {
      log.warn("Client blocked: clientId={}, status={}", clientId, clientVo.getStatus());
    ...
    log.info(
        "User login successful: userId={}, clientId={}, grantType={}, tenantId={}",
        userId,
        clientId,
        grantType,
        tenantId);
  ...
  public R<Void> socialCallback(@RequestBody SocialLoginBody loginBody) {
    log.debug(
        "Social login callback: source={}, tenantId={}",
        loginBody.getSource(),
    ...
    if (!response.ok()) {
      log.warn(
          "Social login failed: source={}, reason={}", loginBody.getSource(), response.getMsg());
      return R.fail(response.getMsg());
    }
    log.info(
        "Social login callback successful: source={}, authId={}",
        loginBody.getSource(),
        authUserData.getUuid());
  ...
  /**
   * Registers a new user account if registration is enabled.
   *
   * @param registerBody registration request containing username, password, and user type
   * @return success response if registration succeeds
   */
  @ApiEncrypt
