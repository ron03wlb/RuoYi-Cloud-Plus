...
public class SysDeptServiceImpl implements ISysDeptService {
    ...
    private LambdaQueryWrapper<SysDept> buildQueryWrapper(SysDeptBo bo) {
        ...
        if (ObjectUtil.isNotNull(bo.getBelongDeptId())) {
            ...
            lqw.and(
                    x -> {
                        List<Long> deptIds =
                                baseMapper.selectDeptAndChildById(bo.getBelongDeptId());
    ...
    public List<Tree<Long>> buildDeptTreeSelect(List<SysDeptVo> depts) {
        ...
        return TreeBuildUtils.buildMultiRoot(
                ...
                (node, treeNode) ->
                        treeNode.setId(node.getDeptId())
                                .setParentId(node.getParentId())
                                .setName(node.getDeptName())
                                .setWeight(node.getOrderNum())
                                .putExtra(
                                        "disabled",
                                        SystemConstants.DISABLE.equals(node.getStatus())));
    }
        ...
        boolean exist =
                baseMapper.exists(
                        new LambdaQueryWrapper<SysDept>()
                                ...
                                .ne(
                                        ObjectUtil.isNotNull(dept.getDeptId()),
                                        SysDept::getDeptId,
                                        dept.getDeptId()));
        return !exist;
    }
