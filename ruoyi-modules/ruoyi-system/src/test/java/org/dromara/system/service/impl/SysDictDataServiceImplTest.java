...
class SysDictDataServiceImplTest {
  ...
  class QueryMethodsTests {
     ...
     *
     * <p>该方法使用 LambdaQueryWrapper.select() 指定返回字段， 在纯单元测试中会抛出 MybatisPlusException。 需要 MyBatis-Plus
     * 表信息初始化才能测试。
     */
    @Test
  ...
  class DeleteMethodsTests {
     ...
     * 注意: 无法测试成功删除场景
     *
     * <p>deleteDictDataByIds() 在删除成功后会调用 CacheUtils.evict()， 这是静态方法且依赖 Spring 上下文初始化。 在纯单元测试中会抛出
