...
public class FlwTaskServiceImpl implements IFlwTaskService {
  ...
  private void wrapAssigneeInfo(List<FlowTaskVo> taskList) {
    ...
    List<User> associatedUsers =
        FlowEngine.userService().getByAssociateds(StreamUtils.toList(taskList, FlowTaskVo::getId));
  ...
  private QueryWrapper<FlowTaskBo> buildQueryWrapper(FlowTaskBo flowTaskBo) {
    ...
    wrapper.like(
        StringUtils.isNotBlank(flowTaskBo.getNodeName()), "t.node_name", flowTaskBo.getNodeName());
    wrapper.like(
        StringUtils.isNotBlank(flowTaskBo.getFlowName()), "t.flow_name", flowTaskBo.getFlowName());
    wrapper.like(
        StringUtils.isNotBlank(flowTaskBo.getFlowCode()), "t.flow_code", flowTaskBo.getFlowCode());
    wrapper.in(
    ...
    if (StringUtils.isNotBlank(flowTaskBo.getCategory())) {
      List<Long> categoryIds =
          flwCategoryMapper.selectCategoryIdsByParentId(Convert.toLong(flowTaskBo.getCategory()));
  ...
  public List<Node> getBackTaskNode(Long taskId, String nowNodeCode) {
    FlowTask task = flowTaskMapper.selectById(taskId);
    List<Node> nodeCodes =
        nodeService.getByNodeCodes(Collections.singletonList(nowNodeCode), task.getDefinitionId());
  ...
  public FlowTaskVo selectById(Long taskId) {
    ...
    if (CollUtil.isNotEmpty(nodeExtVo.getCopySettings())) {
      List<FlowCopyVo> list =
          StreamUtils.toList(nodeExtVo.getCopySettings(), x -> new FlowCopyVo(Convert.toLong(x)));
    ...
    flowTaskVo.setApplyNode(
        flowNode.getNodeCode().equals(flwCommonService.applyNodeCode(task.getDefinitionId())));
    return flowTaskVo;
  }
  ...
  public List<FlowNode> getNextNodeList(FlowNextNodeBo bo) {
    ...
    nextFlowNodes =
        StreamUtils.filter(
            nextFlowNodes, node -> NodeType.BETWEEN.getKey().equals(node.getNodeType()));
    if (CollUtil.isNotEmpty(nextNodeList)) {
      ...
      for (FlowNode flowNode : nextFlowNodes) {
        StreamUtils.findFirst(
                buildNextTaskList, t -> t.getNodeCode().equals(flowNode.getNodeCode()))
            .ifPresent(
                first -> {
                  ...
                  if (CollUtil.isNotEmpty(first.getPermissionList())
                      && CollUtil.isNotEmpty(
                          users =
                              flwTaskAssigneeService.fetchUsersByStorageIds(
                                  StringUtils.joinComma(first.getPermissionList())))) {
                    flowNode.setPermissionFlag(
                        StreamUtils.join(users, e -> Convert.toStr(e.getUserId())));
                  }
                });
      }
    }
