...
public class FlwNodeExtServiceImpl implements NodeExtService, IFlwNodeExtService {
  ...
  static {
    CHILD_NODE_MAP =
        Map.of(
            ...
            Map.of(
                "label", "权限按钮", "type", 4, "must", false, "multiple", true, "desc", "控制该节点的按钮权限"));
  }
  ...
  public List<NodeExt> getNodeExt() {
    ...
    nodeExtList.add(
        buildNodeExt(
            "wf_basic_tab", "基础设置", 1, List.of(CopySettingEnum.class, VariablesEnum.class)));
    // 构建按钮权限页面
    nodeExtList.add(buildNodeExt("wf_button_tab", "权限", 2, List.of(ButtonPermissionEnum.class)));
                ...
                .map(
                    dict ->
                        new ButtonPermissionVo(
                            dict.getValue(), buttonSet.contains(dict.getValue())))
                .toList();
